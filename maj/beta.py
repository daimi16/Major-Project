...
def upload_predict(upload_images, model):
        ...
        y_class = np.argsort(prediction[0])[-5:]
        # y_class = np.argpartition(prediction[0],-5)[-5:]
        print(y_class)
        # y = y_class[0]
        # print(y)
        # res = lab[y]
        # print(res)
        # results=[lab[idx] for idx in y_class]
        results = []
        for idx in reversed(y_class):
              results.append({
                    "name":lab[idx],
                    # "probability":str(prediction[0][idx]*100)
                    "probability":prediction[0][idx].item()
              })
